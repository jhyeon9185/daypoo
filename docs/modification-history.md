...
## [2026-03-23 11:15:00] SupportPage 프리미엄 리액트 디자인 리뉴얼 및 1:1 문의 기능 정상화
...
- **1:1 문의 등록 기능 정상화 (BE/FE)**:
    - **[Critical] 백엔드 버그 수정**: `SupportController.java`에서 소셜 로그인 사용자의 인증 객체 처리 오류(`UserDetails` -> `Authentication`)를 수정하여 500 에러를 해결했습니다.
...
- `backend/src/main/java/com/daypoo/api/controller/SupportController.java`: `createInquiry`, `getMyInquiries` 메서드의 인증 파라미터 타입 수정 및 누락된 임포트 추가.
- `docs/plans/support_redesign_plan.md`: 기능 및 디자인 개편 계획서 작성.

### 결과/영향
- 지원 센터 페이지가 서비스의 '프리미엄' 이미지를 대표하는 고퀄리티 페이지로 업그레이드되었습니다.
- 소셜 로그인 사용자도 오류 없이 1:1 문의를 등록하고 자신의 문의 내역을 확인할 수 있게 되었습니다.
